...
const int start_minute = 58;
const int start_second = 0;
// Hour and minute outputs for display, as well as seconds
int hours = 0;
int minutes = 0;
int seconds = 0;
...
void update_display(int dig1, int dig2, int dig3, int dig4) {
  // Set the first digit
...
void write_message(char message[4]) {
...
// Update the clock's time (hours and minutes)
void update_time() {
  // Calculate the clock second
  seconds = (start_second + millis() / 1000) % 60;
  // Calculate the clock minute
  minutes = (start_minute + (start_second + millis() / 1000) / 60) % 60;
  // Calculate the clock hour
  hours = (start_hour + (start_minute + (start_second + millis() / 1000) / 60) / 60) % 24;
}
...
void loop() {
  // Update the clock's internal time
  update_time();
  ...
  if (hours == 0 && minutes < 1) {         // Display message for 1 min at 12am
    ...
    update_display(hours / 10, hours % 10, minutes / 10, minutes % 10);
  }
}
